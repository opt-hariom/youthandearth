    ...
    if (variantWithInventory.inventory_management && variantWithInventory.inventory_quantity > 0) {
      if (window.PXUTheme.theme_settings.display_inventory_left) {
        ...
        if (variantWithInventory.inventory_quantity <= inventoryThreshold) {
          $('.items_left', $product).html(`<p class="items_left-padding-top">${variantWithInventory.inventory_quantity} ${itemsLeftText}</p>`);
        } else {
...
//feefo Main Product Reviews Styling
function appendStyleToShadowDOM() {
  const style = document.createElement('style');
  style.innerText = `
  * { font-family: Ambit, "Merriweather Sans", sans-serif !important}
  .chip-selected, 
  .feefo-pag-nav-btn.current,
  .on-page-container .rating-filter-select-container .filters-container .reviews-sort button:not(.unselected)
    { background: #fc4301 !important; color: white !important }
  .chip:hover {border-color: #fc4301; color: #003b00 }
  .chip { color: #003b00  }
  .on-page-container .on-page-footer .read-more a { color: #fc4301 }
  .on-product-page-comment-body span,
  .sentiment-snippets-container .snippet-text
  { color: black }
  `;

  let widget;

  if ($('#feefo-product-review-widgetId').length) {
    console.log('product page');
    widget = $('#feefo-product-review-widgetId');
  } else if ($('#feefo-service-review-widgetId').length) {
    console.log('reviews page');
    widget = $('#feefo-service-review-widgetId');
  }

  const shadowRoot = widget[0].shadowRoot;
  if (shadowRoot) {
    console.log("Found Shadow Element")
    shadowRoot.appendChild(style.cloneNode(true));
  } else {
    console.error('Shadow root is not accessible or does not exist.');
  }
}

//feefo Stars Section Styling
function appendStyleToShadowDOMstars() {
  const style = document.createElement('style');
  style.innerText = `.reviews-count { color: black !important; }`;

  const widgets = document.querySelectorAll('.feefo-product-stars-widget');
...
window.onload = function() {
  ...
  appendStyleToShadowDOMstars()
  appendStyleToShadowDOM();
};